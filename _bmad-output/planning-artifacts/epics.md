...
- **FR3:** Validation et association des comptes aux profils métiers (Audit, DM, ETP, DG, Externe).
...
- **FR35:** L'Audit (ou RSSI) paramètre l'organigramme (Directions, Services, Agences) — condition sine qua nu du RBAC.
...
- FR3: Epic 1 - Validation/Association des profils métiers
...
- FR35: Epic 1 - Pilotage de la structure de l'organigramme
...
### Story 1.4: Gestion de l'Organigramme Institutionnel

As a **Audit Interne (ou RSSI)**,
I want **créer et structurer l'organigramme (Directions, Services, Agences)**,
So that **le système reflète fidèlement la structure de la banque.**
...
### Story 1.5: Assignation des Profils Métiers et Périmètres (RBAC)

As a **Audit Interne**,
I want **associer les utilisateurs (DM, ETP) à leurs profils et services respectifs**,
So that **le périmètre de visibilité des données (RBAC) s'applique instantanément.**

**Acceptance Criteria:**

**Given** un compte utilisateur existant,
**When** je lui octroie le rôle "DM" et le lie au département "Risques",
**Then** cet utilisateur ne pourra voir que les données du département "Risques" (FR3, FR28).
...
**When** j'uploade un fichier valide et que je saisis un commentaire,
**Then** l'état FSM passe à `PENDING_DM_REVIEW`.
**And** le fichier est stocké de manière sécurisée et l'extension est validée.
**And** une fois le fichier uploadé, il est impossible de le supprimer physiquement ou de l'altérer en base de données (Append-Only) pour prévenir les manipulations post-soumission (Mitigation Sécurité).
