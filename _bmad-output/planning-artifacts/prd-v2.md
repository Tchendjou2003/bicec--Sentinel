...
**Personas :** Aboubakar N. (RSSI / Support), Jean-Paul M. (Directeur Audit Interne)
...
**Resolution :** L'environnement est opérationnel. L'historique est migré. Les 85 utilisateurs sont formés et connectés. Aboubakar supervise le turnover : quand un collaborateur quitte la BICEC, il désactive le compte technique. Jean-Paul peut également retirer le rôle métier d'un utilisateur, rendant son compte inerte même si l'IT ne l'a pas encore désactivé. La séparation des fonctions (SoD) est garantie : aucun acteur isolé ne peut créer un accès métier fonctionnel.
...
- **Séparation des Fonctions (SoD — ADR-10) :** L'administration technique (IT/RSSI) gère l'infrastructure et crée les comptes utilisateurs comme des « coquilles vides » sans aucun rôle métier. Le Directeur de l'Audit Interne (ou ses délégués) est le **seul habilité** à attribuer les rôles métiers, les habilitations et les périmètres de direction aux comptes créés par l'IT, via une interface dédiée. Aucun acteur isolé ne peut créer un accès métier fonctionnel. L'Audit Interne gère également les délégations/intérims.
